...
### 🌍 Western Name Rule (Translation Prompt v3.3)
- **New `[TÊN TÂY]` Rule**: AI tự nhận diện tên phương Tây phiên âm Trung (杰克=Jack, 迈克尔=Michael, 艾米丽=Emily...) và khôi phục về tên tiếng Anh gốc.
  - SAI: "Kiệt Khắc", "Mạch Khắc Nhĩ" → ĐÚNG: "Jack", "Michael"
  - Phân biệt: Tên Hán thật sự (李明, 张三) → Phiên âm Hán Việt bình thường
  - Nhất quán xuyên suốt: đã dùng "Jack" thì cấm đổi thành "Jeck", "Giắc"
- **Updated `[PHÂN VAI]` Rule**: Tách hướng dẫn cho tên Hán vs tên Tây khi nhân vật không có trong Glossary

### 🔥 Luyện Văn — Global Corrections
- **Unified Correction Pool**: Tất cả corrections giờ dùng chung 1 pool (`__global__`) thay vì per-workspace.
  - Thêm cải chính ở workspace nào → tự động có ở mọi workspace
  - Post-process (string replace), zero API cost
  - Áp dụng cho: tên Tây bị dịch sai, thuật ngữ nhất quán cross-workspace
- **Auto-Migration (DB v106)**: Tự động migrate tất cả corrections cũ sang global pool, deduplicate.
- **All Entry Points Updated**:
  - Dictionary panel, Reader correction dialog, AI inspection fix, Mobile sync — tất cả save vào global pool
  - All query points — tất cả load từ global pool

### 📦 Files Modified
- `lib/gemini/constants.ts` — WESTERN_NAME_RULE + updated PHÂN VAI
- `lib/db.ts` — GLOBAL_WORKSPACE_ID, DB v106 upgrade with migration
- `components/workspace/dictionary/hooks/useCorrections.ts` — Global save/load
- `components/workspace/hooks/useCorrections.ts` — Global save
- `components/workspace/hooks/useReaderInspection.ts` — Global save
- `components/workspace/shared/SyncMobileButton.tsx` — Global save/check
- `components/workspace/chapter-list/ReaderModal.tsx` — Global query
- `components/workspace/chapter-list/hooks/useCorrections.ts` — Global query
