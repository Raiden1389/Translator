...
- **Double-Layer Sanitization**:
  - **Layer 1 (Pre-translation)**: Clean `content_original` before sending to AI
  - **Layer 2 (Post-translation)**: Clean `translatedText` and `translatedTitle` after AI response
- **Database Cleanup**: Added "Dọn HTML rác" button (Eraser icon) in Action Hub
  - Cleans existing translations with HTML tags (`<div>`, `<br>`, etc.)
- **Files**:
  - `lib/utils/text-sanitizer.ts` (NEW) - Centralized sanitization utilities
  - `lib/utils/db-sanitizer.ts` (NEW) - Database cleanup script
  - `TranslationProvider.v2.tsx` - Integrated double-layer sanitization
  - `useChapterImport.ts` - Refactored to use shared utilities
  - `ChapterList.tsx` - Added cleanup button handler

#### **2. Translation Progress - Avg Speed = 0.0 ch/min**
- **Root Cause**: Timer `elapsedSeconds` was reset to 0 when translation finished
- **Fix**: Keep elapsed time when `isTranslating = false`, only reset when starting new translation
- **Implementation**: Track previous `isTranslating` state with ref to detect false → true transition
- **File**: `TranslationProgressOverlay.tsx`

#### **3. Title Translation Failure - Auto-Fix Chinese Characters**
- **Root Cause**: Code threw error when AI didn't translate title (kept Chinese characters)
- **Old Behavior**: `throw new Error("CẢNH BÁO: Phát hiện chữ Hán...")` → Translation FAILED
- **New Behavior**: Auto-fix with `translateTitleOnly()` → Graceful fallback if fix fails
- **Impact**: No more translation crashes due to untranslated titles
- **File**: `lib/gemini/translation/post-processor.ts`

#### **4. Progress Bar Stuck at 67%**
- **Root Cause**: Progress creep stopped when `isTranslating = false`, didn't force 100%
- **Fix**: Force `displayPercent = 100` when `current >= total` and translation finished
- **File**: `TranslationProgressOverlay.tsx`

---

### ✨ New Feature: Dictionary Usage Breakdown

- **Clickable Expansion**: Click "📖 DICTIONARY USAGE" header to expand/collapse per-chapter breakdown
- **Per-Chapter Stats**:
  - Shows each completed chapter with:
    - CH number + title
    - 📚 Glossary terms used
    - 👤 Character personas used
  - Scrollable list (max height 120px)
  - Smooth slide-in animation
- **Backend**:
  - Added `chapterStats` field to progress interface
  - Auto-populated from `progress.progress` Map
  - Only shows completed chapters (status = 'done')
  - Sorted by chapter order
- **Files**:
  - `TranslationProgressOverlay.tsx` - UI + state management
  - `TranslationProvider.v2.tsx` - Data population
  - `useTranslationProgress.ts` - Already tracked per-chapter stats

---

### 📦 Files Modified
- `lib/utils/text-sanitizer.ts` (NEW)
- `lib/utils/db-sanitizer.ts` (NEW)
...
- `components/workspace/hooks/useChapterImport.ts`
